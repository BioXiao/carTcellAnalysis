...
def generate_intra_feature_reads(fastaRecords, numReads=5, minReadLength=20,
                                 maxReadLength=50, end="none"):
    fastqRecords = []
...
def generate_cross_feature_reads(fastaRecords, numReads=5, minReadLength=20,
                                 maxReadLength=50):
    fastqRecords = []
    numRecords = len(fastaRecords)
    for i in range(0, numReads):
        ...
        readLength1 = min(seqLength1,
                          randint(minReadLength, maxReadLength - 10))
        readLength2 = min(seqLength2,
                          randint(max(minReadLength - readLength1, 0),
                                      maxReadLength - readLength1))
        read1 = generate_intra_feature_reads([srcRecord1], 1, readLength1,
                                             readLength1, end="right")[0]
        read2 = generate_intra_feature_reads([srcRecord2], 1, readLength2,
                                             readLength2, end="left")[0]
        ...
        joinedRead.description = (':').join([read1.description,
                                             read2.description])
